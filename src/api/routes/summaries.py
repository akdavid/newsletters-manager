...
async def get_summaries(
    ...
    try:
        with get_db_session() as session:
            query = session.query(SummaryModel).order_by(SummaryModel.generation_date.desc())
...
async def get_summary_details(summary_id: str):
    try:
        with get_db_session() as session:
            summary = session.query(SummaryModel).filter(SummaryModel.id == summary_id).first()
...
async def get_summary_stats():
    try:
        with get_db_session() as session:
            ...
            completed = session.query(SummaryModel).filter(SummaryModel.status == 'completed').count()
            failed = session.query(SummaryModel).filter(SummaryModel.status == 'failed').count()
            sent = session.query(SummaryModel).filter(SummaryModel.status == 'sent').count()
            
            avg_newsletters = session.query(session.func.avg(SummaryModel.newsletters_count)).scalar() or 0
            avg_duration = session.query(session.func.avg(SummaryModel.processing_duration)).scalar() or 0
            
            recent_summary = session.query(SummaryModel)\
            ...
            return {
                ...
                "failed": failed,
                "sent": sent,
                "success_rate": round((completed / total_summaries * 100) if total_summaries > 0 else 0, 2),
                "average_newsletters_per_summary": round(avg_newsletters, 1),
                "average_processing_duration": round(avg_duration, 2),
                "last_summary_date": recent_summary.generation_date.isoformat() if recent_summary else None
            }
