...
from ..models.email import AccountType, Email
...
class EmailCollectorAgent(BaseAgent):
    ...
    async def _setup_subscriptions(self):
        ...
        message_broker.subscribe(
            MessageType.EMAILS_MARKED_READ, self._handle_mark_emails_as_read
        )
    ...
    async def _handle_mark_emails_as_read(self, message):
        ...
        if message.data.get("action") == "mark_as_read":
            ...
            if email_ids:
                self.logger.info(
                    f"Processing request to mark {len(email_ids)} emails as read"
                )
                results = await self.mark_emails_as_read(email_ids)

                # Send a pipeline-specific completion message
                from .base_agent import AgentMessage, MessageType, message_broker
        ...
        try:
            for i, credentials_path in enumerate(self.settings.gmail_credentials_paths):
                if (
                    ...
                    or credentials_path.strip() == ""
                    or credentials_path == "/path/to/gmail2_credentials.json"
                ):
                    self.logger.info(
                        f"Skipping Gmail account {i+1} (no credentials configured)"
                    )
                    continue
                ...
                self.logger.info(
                    f"Gmail account {i+1} ({account_type.value}) initialized successfully"
                )

            # Initialize Outlook service if configured
            if (
                self.settings.outlook_client_id
                and self.settings.outlook_client_id != "your_outlook_client_id"
                and self.settings.outlook_client_secret
                and self.settings.outlook_client_secret != "your_outlook_client_secret"
            ):
    ...
    async def execute(self, max_emails_per_account: int = None) -> Dict[str, Any]:
        ...
        await self.publish_message(MessageType.EMAIL_COLLECTED, result)

        self.logger.info(
            f"Email collection completed: {len(collected_emails)} emails collected"
        )
        return result

    async def _collect_from_gmail(
        self, gmail_service: GmailService, max_emails: int
    ) -> List[Email]:
        try:
            self.logger.info(
                f"Collecting emails from {gmail_service.account_type.value}"
            )
            ...
            for message_info in message_list:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to get details for Gmail message {message_info['id']}: {e}"
                    )
                    continue

            self.logger.info(
                f"Collected {len(emails)} emails from {gmail_service.account_type.value}"
            )
            return emails

        except Exception as e:
            self.logger.error(
                f"Gmail collection failed for {gmail_service.account_type.value}: {e}"
            )
            raise
    ...
    async def _collect_from_outlook(self, max_emails: int) -> List[Email]:
        try:
            ...
            for message_info in message_list:
                try:
                    email = await self.outlook_service.get_message_details(
                        message_info["id"]
                    )
                    if email:
                        emails.append(email)
                    ...
                    self.logger.warning(
                        f"Failed to get details for Outlook message {message_info['id']}: {e}"
                    )
                    continue
            ...
            with db_manager.get_session() as session:
                for email in emails:
                    existing = (
                        ...
                        .filter_by(message_id=email.message_id)
                        .first()
                    )
                    ...
                    if not existing:
                        ...
                        self.logger.debug(
                            f"Email {email.message_id} already exists in database"
                        )
        ...
        try:
            with db_manager.get_session() as session:
                for email_id in email_ids:
                    email_model = (
                        session.query(EmailModel).filter_by(id=email_id).first()
                    )
                    if not email_model:
                    ...
                    if email_model.account_source.startswith("gmail"):
                        ...
                        if service_index < len(self.gmail_services):
                            success = await self.gmail_services[
                                service_index
                            ].mark_message_as_read(email_model.message_id)
                        else:
                            self.logger.error(
                                f"Gmail service index {service_index} not available"
                            )
                        ...
                        if self.outlook_service:
                            ...
                            outlook_id = (
                                email_model.provider_id or email_model.message_id
                            )
                            ...
                            if not success and email_model.provider_id:
                                self.logger.warning(
                                    f"Failed with provider_id, trying message_id for {email_id}"
                                )
                                success = (
                                    await self.outlook_service.mark_message_as_read(
                        ...
                        self.logger.error(
                            f"Unknown account source: {email_model.account_source}"
                        )
                    ...
                    if success:
                        ...
                        successful_marks += 1
                        self.logger.debug(
                            f"Successfully marked email {email_id} as read"
                        )
                    else:
                ...
                session.commit()
                self.logger.info(
                    f"Successfully marked {successful_marks}/{len(email_ids)} emails as read"
                )
