...
async def test_outlook_connection():
    ...
    print(
        f"🆔 Client ID: {client_id[:8]}...{client_id[-8:] if client_id else 'Not set'}"
    )
    print(
        f"🔑 Client Secret: {'✅ Set' if client_secret and client_secret != 'your_outlook_client_secret' else '❌ Not configured'}"
    )
    print(
        f"🏢 Tenant ID: {tenant_id[:8]}...{tenant_id[-8:] if tenant_id else 'Not set'}"
    )
    print()
    ...
    try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        mail_address = user_data.get(
                            "mail", user_data.get("userPrincipalName", "No email")
                        )
                        print(
                            f"✅ User: {user_data.get('displayName', 'Unknown')} ({mail_address})"
                        )
                        ...
                        if (
                            ...
                            and "hotmail"
                            in str(user_data.get("userPrincipalName", "")).lower()
                        ):
                            print(
                                "⚠️  Note: Personal accounts sometimes have limited Graph API mail access"
                            )
                            print(
                                "💡 Your account might need to be accessed differently"
                            )
                    else:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        print(
                            f"✅ Found {len(folders_data.get('value', []))} mail folders"
                        )
                    else:
                        error_text = await response.text()
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(
                        ...
                        if response.status == 200:
                            ...
                            print(
                                f"✅ Manual API call successful: Found {len(messages)} messages"
                            )
                        else:
                            error_text = await response.text()
                            print(
                                f"❌ Messages API error {response.status}: {error_text}"
                            )
        ...
        if "AADSTS" in error_msg:
            ...
            if "invalid_client" in error_msg:
                ...
                print(
                    "- Check API permissions in Azure Portal (Mail.Read, Mail.ReadWrite)"
                )
