...
from ..utils.helpers import (
    extract_domain, has_unsubscribe_link, contains_tracking_pixels,
    contains_promotional_keywords, calculate_html_to_text_ratio
)
...
class NewsletterDetectorAgent(BaseAgent):
    ...
    def _basic_newsletter_detection(self, email: Email) -> tuple[float, List[DetectionMethod]]:
        ...
        newsletter_patterns = [
            'newsletter', 'noreply', 'no-reply', 'donotreply', 'digest',
            'update', 'notification', 'bulletin', 'news'
        ]
        ...
        commercial_domains = [
            ...
            'amazonses.com', 'mailgun.org', 'postmarkapp.com'
        ]
    ...
    async def _calculate_sender_frequency_score(self, sender: str) -> float:
        try:
            ...
            self.logger.warning(f"Failed to calculate sender frequency for {sender}: {e}")
            return 0.0
        ...
        try:
            with db_manager.get_session() as session:
                ...
                if not existing:
                    ...
                    email_model = session.query(EmailModel).filter_by(id=newsletter.email_id).first()
                    if email_model:
                        email_model.is_newsletter = True
                    
                    session.commit()
                    self.logger.debug(f"Stored newsletter for email {newsletter.email_id}")
