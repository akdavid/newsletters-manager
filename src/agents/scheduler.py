...
class SchedulerAgent(BaseAgent):
    ...
    async def _initialize_scheduler(self):
        try:
            ...
            self.logger.info(f"Scheduler started with daily summary at {self.settings.daily_summary_time}")
    ...
    async def _run_daily_summary(self):
        ...
        try:
            if not self.orchestrator_agent:
                self.logger.error("Orchestrator agent not set, cannot run daily summary")
                return
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if self.scheduler:
            ...
            if daily_job and daily_job.next_run_time:
                scheduler_health["next_daily_summary"] = daily_job.next_run_time.isoformat()
