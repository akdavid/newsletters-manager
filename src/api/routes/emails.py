...
async def get_emails(
    ...
    is_newsletter: Optional[bool] = Query(None, description="Filter by newsletter status"),
...
async def get_email_stats():
    try:
        with get_db_session() as session:
            ...
            newsletters = session.query(EmailModel).filter(EmailModel.is_newsletter == True).count()
            processed = session.query(EmailModel).filter(EmailModel.is_processed == True).count()
            unread = session.query(EmailModel).filter(EmailModel.status == 'unread').count()
            
            by_account = {}
            accounts = session.query(EmailModel.account_source, session.query(EmailModel).filter(EmailModel.account_source == EmailModel.account_source).count().label('count')).group_by(EmailModel.account_source).all()
            ...
            return {
                ...
                "newsletter_percentage": round((newsletters / total_emails * 100) if total_emails > 0 else 0, 2),
                "processed_percentage": round((processed / total_emails * 100) if total_emails > 0 else 0, 2)
            }
