...
from ..db.models import EmailModel, NewsletterModel, SummaryModel
from ..models.email import Email
from ..models.newsletter import Newsletter
from ..models.summary import NewsletterSummaryItem, Summary, SummaryStatus
...
class ContentSummarizerAgent(BaseAgent):
    ...
    async def _setup_subscriptions(self):
        self.subscribe_to_message(
            MessageType.NEWSLETTER_DETECTED, self._handle_newsletters_detected
        )
    ...
    async def execute(self, newsletters: List[Newsletter] = None) -> Summary:
        try:
            ...
            self.logger.info(
                f"Starting summarization for {len(newsletters)} newsletters"
            )
            ...
            for newsletter in newsletters:
                ...
                if email:
                    summary_item = await self.ai_service.summarize_newsletter(
                        email, newsletter
                    )
                    if summary_item:
            ...
            daily_summary = await self.ai_service.generate_daily_summary(
                newsletter_summaries
            )
            daily_summary.processing_duration = (
    ...
    async def _get_emails_for_newsletters(
        ...
        try:
            with db_manager.get_session() as session:
                email_models = (
                    session.query(EmailModel).filter(EmailModel.id.in_(email_ids)).all()
                )
    ...
    def _model_to_email(self, model: EmailModel) -> Email:
        from ..models.email import AccountType, EmailAttachment, EmailStatus
            ...
            with db_manager.get_session() as session:
                summary_model = SummaryModel(
                    ...
                    newsletters_summaries=[
                        item.to_dict() for item in summary.newsletters_summaries
                    ],
    ...
    async def _mark_emails_as_processed(self, newsletters: List[Newsletter]):
        try:
            ...
            with db_manager.get_session() as session:
                session.query(EmailModel).filter(EmailModel.id.in_(email_ids)).update(
                    {
                        EmailModel.is_processed: True,
                        EmailModel.updated_at: datetime.now(timezone.utc),
                    }
                )
    ...
    async def _update_summary_status(
        self, summary_id: str, status: SummaryStatus, error_message: str = None
    ):
        try:
            with db_manager.get_session() as session:
                summary_model = (
                    session.query(SummaryModel).filter_by(id=summary_id).first()
                )
                if summary_model:
    ...
    def _model_to_summary(self, model: SummaryModel) -> Summary:
        from ..models.summary import NewsletterSummaryItem, SummaryFormat, SummaryStatus
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        summarizer_health = {
            ...
            "smtp_configured": bool(
                self.settings.smtp_username and self.settings.smtp_password
            ),
