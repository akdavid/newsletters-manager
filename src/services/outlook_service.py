...
class OutlookService:
    ...
    async def authenticate(self, username: str = None):
        try:
            ...
            if accounts:
                logger.info(
                    f"Found {len(accounts)} cached account(s), attempting silent authentication..."
                )
                ...
                if result and "access_token" in result:
                    ...
                    self.token_expires_at = datetime.now().timestamp() + result.get(
                        "expires_in", 3600
                    )
                    self._save_cache()
                    logger.info(
                        f"Outlook service authenticated silently for account: {account.get('username', 'unknown')}"
                    )
                    return
                else:
                    logger.info(
                        "Silent authentication failed, falling back to interactive login"
                    )
            else:
            ...
            if "access_token" in result:
                ...
                self._save_cache()
                logger.info(
                    "Outlook service authenticated successfully via device flow"
                )
            else:
    ...
    async def _ensure_token_valid(self):
        if not self.access_token or (
            ...
            and datetime.now().timestamp() >= self.token_expires_at
        ):
